...
function generateRepositories(config, controllers) {
    const dataDir = path.join(config.dest, conf.dataDir);
    const reposDir = path.join(dataDir, 'repositories');
    (0, utils_1.createDir)(dataDir);
    (0, utils_1.createDir)(reposDir);
    const repoExports = [];
    controllers.forEach(controller => {
        ...
        controller.methods.forEach(m => {
            ...
            methodSignatures.push(`  abstract ${simpleName}(${paramsArg}): Observable<${responseType}>;`);
            const callArgs = hasParams ? 'params' : '';
            methodImplementations.push(`  ${simpleName}(${paramsArg}): Observable<${responseType}> {\n` +
        ...
        (0, utils_1.writeFile)(abstractFile, abstractContent, config.header);
        (0, utils_1.writeFile)(implFile, implContent, config.header);
    ...
    (0, utils_1.writeFile)(reposIndexFile, reposBarrel, config.header);
    // Generate data barrel
    const dataIndexFile = path.join(dataDir, 'index.ts');
    (0, utils_1.writeFile)(dataIndexFile, `export * from './repositories';\n`, config.header);
}
exports.generateRepositories = generateRepositories;
//# sourceMappingURL=generate-repositories.js.map