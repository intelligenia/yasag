 ...
 */
export function normalizeSwagger1(schema: any): any {
  const result: any = {
    swagger: '2.0',
    info: {
      title: schema.info?.title || schema.resourcePath || 'API',
  ...
  if (schema.models) {
    Object.entries(schema.models).forEach(([modelName, modelDef]: [string, any]) => {
 ...
 */
function normalizeModel(model: any): any {
  const def: any = {
    type: 'object',
    properties: {},
  };

  if (model.description) def.description = model.description;
  if (model.required) def.required = model.required;

  if (model.properties) {
    Object.entries(model.properties).forEach(([propName, propDef]: [string, any]) => {
 ...
 */
function normalizeModelProperty(prop: any): any {
  const result: any = {};
  ...
  if (prop.defaultValue !== undefined) result.default = prop.defaultValue;
  ...
  if (prop.pattern !== undefined) result.pattern = prop.pattern;
  if (prop.minItems !== undefined) result.minItems = prop.minItems;
  if (prop.maxItems !== undefined) result.maxItems = prop.maxItems;
  if (prop.uniqueItems !== undefined) result.uniqueItems = prop.uniqueItems;
  if (prop.readOnly !== undefined) result.readOnly = prop.readOnly;
 ...
 */
function mapSwagger1Type(type: string): { type: string; isRef: boolean } {
 ...
 */
function normalizeOperation(op: any, api: any, schema: any): any {
  const result: any = {
    tags: [],
  ...
  if (op.parameters && Array.isArray(op.parameters)) {
    result.parameters = op.parameters.map((param: any) => normalizeParameter(param));
  }
  ...
  if (op.responseMessages && Array.isArray(op.responseMessages)) {
    op.responseMessages.forEach((rm: any) => {
      if (rm.code) {
  ...
  // Produces/consumes
  if (op.produces) result.produces = op.produces;
  if (op.consumes) result.consumes = op.consumes;
 ...
 */
function normalizeParameter(param: any): any {
  const result: any = {
    name: param.name || '',
  ...
  if (param.pattern !== undefined) result.pattern = param.pattern;
  if (param.minItems !== undefined) result.minItems = param.minItems;
  if (param.maxItems !== undefined) result.maxItems = param.maxItems;
  if (param.uniqueItems !== undefined) result.uniqueItems = param.uniqueItems;
