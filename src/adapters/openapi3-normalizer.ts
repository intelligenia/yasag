 ...
 */
export function normalizeOpenApi3(schema: any): any {
 ...
 */
function parseServers(schema: any, result: any) {
  ...
  if (variables && typeof serverUrl === 'string') {
    Object.entries(variables).forEach(([varName, varDef]: [string, any]) => {
      const defaultValue = varDef.default || varDef.enum?.[0] || varName;
 ...
 */
function normalizeSchemas(schemas: any): any {
  const definitions: any = {};

  Object.entries(schemas).forEach(([name, schemaDef]: [string, any]) => {
 ...
 */
function normalizeSchemaObject(schema: any): any {
  ...
  if (result.anyOf && Array.isArray(result.anyOf)) {
    ...
    const hasNull = result.anyOf.some((s: any) => s.type === 'null');
  ...
  if (result.discriminator) {
    if (typeof result.discriminator === 'object') {
      ...
      if (result.discriminator.mapping) {
        ...
        Object.entries(result.discriminator.mapping).forEach(([key, ref]: [string, any]) => {
  ...
  if (result.properties) {
    const newProps: any = {};
    Object.entries(result.properties).forEach(([propName, propDef]: [string, any]) => {
...
function mergeAllOf(schemas: any[]): any {
  ...
  schemas.forEach(s => {
    ...
    if (normalized.type) merged.type = normalized.type;
    if (normalized.description) merged.description = normalized.description;
 ...
 */
function resolveComponentRef(ref: string, components: any): any {
 ...
 */
function normalizePaths(paths: any, components: any): any {
  const result: any = {};
  ...
  Object.entries(paths).forEach(([pathUrl, pathItem]: [string, any]) => {
    ...
    if (pathItem.$ref) {
      ...
      if (resolved) {
        resolvedPathItem = { ...resolved, ...pathItem };
    ...
    if (resolvedPathItem.parameters) {
      result[pathUrl].parameters = resolvedPathItem.parameters.map(
        (p: any) => normalizeParameter(p, components)
      );
    }
 ...
 */
function normalizeOperation(operation: any, components: any): any {
  const result: any = {
    ...operation,
    parameters: [],
    responses: {},
  ...
  if (operation.parameters) {
    result.parameters = operation.parameters.map(
      (p: any) => normalizeParameter(p, components)
    );
  }
 ...
 */
function normalizeParameter(param: any, components: any): any {
  ...
  if (result.schema) {
    ...
    if (schema.readOnly !== undefined) result.readOnly = schema.readOnly;
    if (schema.writeOnly !== undefined) result.writeOnly = schema.writeOnly;
    if (schema.example !== undefined) result.example = schema.example;
    if (schema['x-nullable']) result['x-nullable'] = true;
...
function normalizeRequestBody(requestBody: any): any[] {
  ...
  const wildcard = content['*/*'] || Object.values(content)[0] as any;
 ...
 */
function normalizeFormData(mediaType: any, required: boolean, description: string): any[] {
  ...
  if (schema.properties) {
    ...
    return Object.entries(schema.properties).map(([name, propDef]: [string, any]) => {
 ...
 */
function normalizeResponses(responses: any, components: any): any {
  const result: any = {};

  Object.entries(responses).forEach(([code, responseDef]: [string, any]) => {
    ...
    if (resolved.content) {
      const mediaType = resolved.content['application/json'] ||
                        ...
                        resolved.content['*/*'] ||
                        Object.values(resolved.content)[0] as any;
 ...
 */
function ensureOperationIds(result: any) {
