...
export class PetUploadImageFormService {
  ...
  submit(value: any = false): Observable<__model.ApiResponse> {
    ...
    this.cacheSub[cacheKey] = new ReplaySubject<__model.ApiResponse>(1);
  ...
  try(subject: ReplaySubject<__model.ApiResponse>, value: any, cache_hit: boolean, cache: boolean, cacheKey: string, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    const result = this.petService.uploadImage(value, this.multipart);
  ...
  listen(value: any = false, submit: boolean = true): Observable<__model.ApiResponse> {
    ...
    if (!this.apiConfigService.listeners[this.cache + JSON.stringify(cacheValue)]) {
      this.apiConfigService.listeners[this.cache + JSON.stringify(cacheValue)] = {fs: this, payload: cacheValue, subject: new ReplaySubject<__model.ApiResponse>(1)};
    }
    if (submit) {
     this.submit(value);
    }
