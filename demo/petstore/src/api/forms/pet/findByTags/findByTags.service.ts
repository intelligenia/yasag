...
@Injectable()
export class PetFindByTagsFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.Pet[]> {
    ...
    this.cacheSub[JSON.stringify(value)] = new ReplaySubject<__model.Pet[]>(1);
  ...
  try(subject: ReplaySubject<__model.Pet[]>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value)];
      return;
    }
    const result = this.petService.findByTags(value);
    result.pipe(
      map(val => {
