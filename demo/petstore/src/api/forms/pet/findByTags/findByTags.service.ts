...
@Injectable()
export class PetFindByTagsFormService extends YASAGGetFormService<__model.Pet[]> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: PetService,
  ) {
    super('PetFindByTags', apiConfigService, ngZone);
  ...
  submit(value: any = false, cache = true, only_cache = false): Observable<__model.Pet[]> {
    const result = this.service.findByTags(value || this.form.value, this.multipart);
