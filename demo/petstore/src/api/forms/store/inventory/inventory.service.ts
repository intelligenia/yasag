...
export class StoreInventoryFormService {
  ...
  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<{[key: string]: number}> {
    ...
    this.cacheSub[cacheKey] = new ReplaySubject<{[key: string]: number}>(1);
  ...
  try(subject: ReplaySubject<{[key: string]: number}>, value: any, cache_hit: boolean, cache: boolean, cacheKey: string, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
  ...
  listen(value: any = false, submit: boolean = true): Observable<{[key: string]: number}> {
    ...
    if (!this.apiConfigService.listeners[this.cache + JSON.stringify(cacheValue)]) {
      this.apiConfigService.listeners[this.cache + JSON.stringify(cacheValue)] = {fs: this, payload: cacheValue, subject: new ReplaySubject<{[key: string]: number}>(1)};
    }
