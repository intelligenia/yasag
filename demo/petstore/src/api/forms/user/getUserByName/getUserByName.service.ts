...
export class UserGetUserByNameFormService {
  ...
  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.User> {
    ...
    this.cacheSub[JSON.stringify(value) + cache] = new ReplaySubject<__model.User>(1);
  ...
  try(subject: ReplaySubject<__model.User>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
