...
@Injectable()
export class UserGetUserByNameFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.cacheSub = {};
    this.cache = 'UserGetUserByName';
  }

  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.User> {
    if (value === false) {
    ...
    this.cacheSub[cacheKey] = new ReplaySubject<__model.User>(1);
  ...
  try(subject: ReplaySubject<__model.User>, value: any, cache_hit: boolean, cache: boolean, cacheKey: string, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
  ...
  listen(value: any = false): Observable<__model.User> {
    if (value === false) {
      value = this.form.value;
    }
    if(!this.apiConfigService.listeners[this.cache + JSON.stringify(value)]){
      this.apiConfigService.listeners[this.cache + JSON.stringify(value)] = {fs: this, payload: value, subject: new ReplaySubject<__model.User>(1)};
    }
