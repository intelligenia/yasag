...
@Injectable()
export class UserUpdateUserFormService extends YASAGPostFormService<string> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: UserService,
  ) {
    super('UserUpdateUser', apiConfigService, ngZone);
  ...
  submit(value: any = false): Observable<string> {
    const result = this.service.updateUser(value || this.form.value, this.multipart);
